...
def get_cache_dir() -> str:
    """Get cache directory: ~/.cache/opensora/models/"""
    cache_dir = os.path.join(os.path.expanduser("~"), ".cache", "opensora", "models")
    os.makedirs(cache_dir, exist_ok=True)
    return cache_dir


def get_cached_model_path(original_path: str) -> str:
    """Get cached model path if valid, otherwise None"""
    cache_dir = get_cache_dir()
    path_hash = hashlib.sha256(original_path.encode()).hexdigest()[:16]  # Short hash
    cached_path = os.path.join(cache_dir, f"cached_{path_hash}.safetensors")
    
    # Simple validation: check if cached file exists and original file exists
    if (os.path.exists(cached_path) and os.path.exists(original_path) and 
        os.path.getsize(cached_path) == os.path.getsize(original_path)):
        return cached_path
    return None


def cache_model_file(original_path: str) -> str:
    """Cache model file for faster loading"""
    cache_dir = get_cache_dir()
    path_hash = hashlib.sha256(original_path.encode()).hexdigest()[:16]
    cached_path = os.path.join(cache_dir, f"cached_{path_hash}.safetensors")
    
    # Check if already cached
    existing = get_cached_model_path(original_path)
    if existing:
        return existing
...
def load_checkpoint(
    ...
    Args:
        ...
        use_cache (bool): Whether to use caching for faster loading.
    ...
    # Use caching for safetensors files if enabled
    if use_cache and path.endswith(".safetensors"):
        cached_path = get_cached_model_path(path)
        path = cached_path if cached_path else cache_model_file(path)
